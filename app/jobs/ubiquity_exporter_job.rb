class UbiquityExporterJob < ActiveJob::Base
  ...
  def perform(tenant_name, export_type, requester_id)
    model_list =  Hyrax.config.curation_concerns
    ...
    if  'all_database'.downcase == export_type
      ...
      orchestrator  = Ubiquity::CsvExportOrchestrator.new(csv_data: csv_data, csv_filename: export_type, tenant_name: tenant_name, requester: requester_id)
      ...
      puts "inside-model"
      csv_data = Ubiquity::CsvGenerator.new.export_remap_model(export_type)
      orchestrator  = Ubiquity::CsvExportOrchestrator.new(csv_data: csv_data, csv_filename: export_type, tenant_name: tenant_name, requester: requester_id)
      orchestrator.upload_to_s3
