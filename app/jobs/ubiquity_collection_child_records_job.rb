class UbiquityCollectionChildRecordsJob < ActiveJob::Base
  def perform(collection_id: , collection_name: nil, tenant_name:, work_id: nil,  type: )
    AccountElevator.switch!(tenant_name)
    if type == 'remove'
      Ubiquity::ManageCollectionChildRecords.remove_member_objects(collection_id, collection_name)
    elsif type == "update"
