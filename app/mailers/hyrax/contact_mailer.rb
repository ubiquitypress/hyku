module Hyrax
  ...
  class ContactMailer < HykuMailer #ActionMailer::Base
    def contact(contact_form, account = nil)
      @contact_form = contact_form
       account = account || Account.find_by(tenant: Apartment::Tenant.current)

      # Check for spam
      return if @contact_form.spam?
      headers = @contact_form.headers.dup
      headers[:subject] << " [#{host_for_tenant}]"
      headers[:to] = account.try(:contact_email) || Hyrax.config.contact_email
