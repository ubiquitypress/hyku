class API::V1::HighlightsController < ActionController::Base
  ...
  def index
    ...
    last_modified = [@collections.last.try(:date_modified).try(:to_time), @featured_works.last.try(:date_modified).try(:to_time), @recent_documents.last.try(:date_modified).try(:to_time)].compact.max
